# HANDOVER · Agent_ERP 参考应用档案（支）

> **治理约定（2026-09-11）**  
> 本目录**只描述支** —— `apps/agent-erp` 参考应用（销售订单 → 出货 PoC）。  
> **不描述 X-Agent 主框架**。框架文档见仓库根 [`README.md`](../README.md) 与 [`packages/core/`](../packages/core/README.md)。

| 主 / 支 | 路径 | 文档 |
|---|---|---|
| **主** X-Agent | `packages/core/` | `packages/core/*.md` |
| **支** Agent_ERP | `apps/agent-erp/` | **本 `HANDOVER/`** |

接手 ERP 应用开发：从 [`00_START_HERE.md`](./00_START_HERE.md) 读起。  
接手框架 / 协议：去读 `packages/core/`，不要从 HANDOVER 推断框架边界。
