...
**档案版本**：v1.1 · 2026-09-06（对齐 GitHub `main` 与防漂移规则）
...
| 本地运行 | ⚠️ 按需启动 | `cd app && npm run serve` → `http://localhost:3001`（勿默认假设进程已在跑） |
| 源码版本 | ✅ GitHub `main` | 远端 `EnglandTong/Agent_ERP`；早期沙箱 5 提交仅存于 `Agent_ERP.bundle` |
| 远程仓库 | ✅ **已建** | `https://github.com/EnglandTong/Agent_ERP.git` |
| 密钥文件 | ✅ `.env` 不进 Git | 只用 `.env.example`；Key 写 `.env.local` |
| 档案规模 | ✅ 11 份 | 合计约 **1010** 行（含空行；以仓库内文件为准，勿抄旧「1223」） |
...
| **P2** | 推送本轮档案对齐提交；阶段结束重打 zip/bundle 放网盘 | 见 `07_OPS.md` 备份清单 |
