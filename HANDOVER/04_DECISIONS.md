...
| 11 | 备份 | zip + git + **GitHub 远程**（`EnglandTong/X-Agent`，原 `Agent_ERP`） | 只放云端沙箱 |
...
## 十一、主仓 / 分支仓 / 骨干三层定位（2026-09-07 定 · 2026-09-11 落地为 X-Agent monorepo）

| 层 | 定位 | 现实对应（2026-09-11） |
|---|---|---|
| **主 · X-Agent** | 协议与类型（`@x-agent/core`） | ✅ `packages/core/` · 文档见 `packages/core/*.md` |
| **骨干 =「AI 的人」** | 脑袋 + Agent 循环 + 感官 + 辨认 | 🟡 运行时仍在 `apps/agent-erp/src/server/`；逐步抽到 `packages/` |
| **支 · Agent_ERP** | 试验田 —— 验证框架可不可用 | ✅ `apps/agent-erp/` · 文档见 `HANDOVER/` |
...
- ⛔ **不用 Git 分支把主支硬拆** —— 同 `main` + 目录分工（决策 #29）；守「核心闭环未通不加第二个大功能」
- ⛔ 不为远期感官写任何代码 —— 只在协议里留 `modality` 开放枚举
- ⛔ **不在 HANDOVER 里写框架规格** —— 主叙事只在 `packages/core/` 与根 `README.md`
