# 11 · 接地层：个人用语（A）与企业别名（B）

> Phase 2 总工单见 [`13_PHASE2_GROUNDING.md`](./13_PHASE2_GROUNDING.md)。Owner **2026-09-11 选 C**：A+B 都要，B 须审核。
...
```
个人用语（A, active）  >  企业别名（B, active）  >  code/model 精确  >  主数据 fuzzy  >  模型抽取  >  规则兜底
```
...
| 静默学习每一次输入 | 口误会污染词表 |
| 静默写入企业别名 | B 层须 `candidate` + 人工 `approve`（#30）；与 A 分表 |
| 让模型维护词表 | 词表是确定性数据，由 API + 确认卡写入 |
| **记标准名**（客户 name / code、产品 model / name） | 系统本来就认得，记了只污染词表。写入前经 `isStandardTerm()`：命中则**跳过**并返回 `skipped:'standard_term'` —— 是"不需要"不是"失败"，前端必须如实说明 |
