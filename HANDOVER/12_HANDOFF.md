...
**当前真实状态**：后端完整且实跑验证过（12 动词、云端模型 100%/100%）；**但"人用过的痕迹"几乎为零**（用语表 0 行、画布 0 行）；**语音本地化实际进度 0%**（模型目录只有 7KB）。

**所以交给你的不是"继续开发"，而是"把已有的东西跑起来验证，再补语音"。**
...
### 🚨 本地领先远端 6 个提交，全部没 push

```
远端 origin/main = f1eaa81
本地 HEAD        = 7c69bae
未推送：134af10 / 36d24b0 / 5971f49 / 02e3d77 / a759a7c / 7c69bae
```
...
### B · 代码完成但从没真跑过 ⚠️ **这才是真短板**

| 项 | 证据 |
|---|---|
| 个人用语表 | `PersonalLexeme` **0 行** |
| 画布面板 | `Panel` **0 行** |
| Owner 10 条真试用 | 跑过必然产生用语数据 → 推断**没跑** |
| UI 点「记住」手感 | 档案自述「需浏览器」 |

**系统能跑，但没人验证过"人用起来顺不顺"。** 这类项目死在这一步的比死在技术上的多得多。
...
### T1 · 端到端真链路验证（Owner 亲自跑，30–60 分钟）
...
`eval/asr-utterances.jsonl` 只有 **3 条**，统计上无意义。
补到 **≥30 条**，必须包含：客户名（含同音姓氏 张/章）、产品编码（A-100/B-200）、
数量+单位、日期口语（下周三/月底）、仓库名。
**有条件的用真实录音，没有就用 TTS 合成 + 人工校对转写文本。**

### T4 · code 消解收紧（阶段 2-P1，唯一剩下的开发项）

`resolve.ts` 的 fuzzy 当前是相似度 0.5 / 分差 0.08，没有 code 优先。
目标：主数据按 code 精确优先，重名必问；与个人用语表共存（**个人用语优先于 fuzzy**）。

**验收**：`npm run eval` 不劣化（模型 100%/100%，规则 ≥96%/98%）；新加 3–5 条口吻进 `eval/utterances.jsonl`。
...
| # | 决策 | 当前默认 |
|---|---|---|
| D1 | 主干落点 | ✅ 已定：Agent_ERP 内抽 `core/`，触发条件满足再独立仓 |
| D2 | 首场景 | ✅ 已定：语音开单 |
